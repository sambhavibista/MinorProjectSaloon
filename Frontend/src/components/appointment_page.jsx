...
const AppointmentPage = () => {
  ...
  const [stylistList, setStylistList] = useState(initialStylistList);
  const [selectedStylist, setSelectedStylist] = useState(null);

  useEffect(() => {
    const interval = setInterval(() => {
      // Randomly update availability
      const updatedStylists = stylistList.map(stylist => ({
        ...stylist,
        available: Math.random() > 0.5, // 50% chance of changing availability
      }));
      setStylistList(updatedStylists);
  ...
  const handleStylistChange = (stylist) => {
    setSelectedStylist(stylist); // Update selected stylist in parent component
    // Handle any other logic related to selecting a stylist
  };
